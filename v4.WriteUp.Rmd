...
DimPlot(wbm, label = T, repel = T, cols = color_pal,
        pt.size = .001) +
  theme_bw() + NoLegend() +
...
DimPlot(temp.subset, cols = color_pal, 
        split.by = 'Condition3', ncol =2, pt.size =  0.05) +
  theme_bw() +  
  theme(text = element_text(size = 6, family = 'sans'))
...
temp.table <- as.data.frame(table(as.character(temp.obj$Condition3), temp.obj$figure.labels))
...
```{r}
temp.table <- as.data.frame(table(wbm$Condition3, wbm$figure.labels))
...
# Cluster Correlation 40xGenes Delta of pairwise cluster centroid
...
I think most of this may be done on the command line.
