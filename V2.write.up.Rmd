...
new.mrg <- mrg2[,columns.to.keep]

dim(new.mrg)

new.mrg[1:5,1:5]

d <- dist(new.mrg, method = 'euclidean')
hc1 <- hclust(d, method = 'complete')

sub_grp <- cutree(hc1, k = 50)
summary(as.factor(sub_grp))

x <- kmeans(new.mrg, 5)
summary(as.factor(x$cluster))

group3 <- names(x$cluster[x$cluster == 3])

temp.df <- new.mrg[group3,]

temp.df2 <- reshape2::melt(as.matrix(temp.df))

temp.df3 <- new.mrg[group3, key.columns]

temp.df3 <- reshape2::melt(as.matrix(temp.df3))

mid.val <- mean(temp.df3$value)

ggplot(temp.df3, aes(x = Var2, y = Var1, fill = value)) +
  geom_tile() +
  scale_fill_gradient2(low = 'blue', mid = 'white', high = 'red',
                       midpoint = mid.val)
```
