...
wbm$Experiment <- ifelse(wbm$Condition2 %in% c('Migr1','enrMigr1'), 'Migr1','Mpl')

wbm$Enrichment <- ifelse(wbm$Condition2 %in% c('enrMigr1','enrMpl'), 'Enriched', 'Not Enriched')
```
...
## w/in Clusters fine labels

Combined Enriched and non-enriched
...
```{r}
de.genes <- read.xlsx('./data/v3/within.cluster.DEGs.Migr1.vs.Mpl.xlsx',
...
wbm$exp <- ifelse(wbm$Condition2 %in% c('Migr1','enrMigr1'), 'Migr1','Mpl')

mep.mcp.de.genes <- de.genes[de.genes$cluster == 'MEP/MCP',]

mep.mcp.de.genes <- mep.mcp.de.genes[order(mep.mcp.de.genes$avg_logFC, decreasing = T),]$gene

DoHeatmap(subset(wbm, v6.labels == 'MEP/MCP',), features = mep.mcp.de.genes, label = F,
          group.bar = T, group.by = 'exp')

all.genes <- de.genes[order(de.genes$cluster, de.genes$avg_logFC, decreasing = T),]$gene


```

## w/in Clusters general labels

Combining enriched and non-enriched
...
#                        gene = character())
# 
# clusters.that.have.log2fc.results <- unique(wbm$v7.labels)[unique(wbm$v7.labels) != '']
# 
# for (i in clusters.that.have.log2fc.results){
#   print(i)
#   temp.subset <- subset(wbm, v7.labels == i)
#   x <- FindMarkers(temp.subset,
#                    ident.1 = 'Migr1',
#                    ident.2 = 'Mpl',
#                    group.by = 'Experiment',
...
#   temp.name <- gsub('/','_',temp.name)
#   print(temp.name)
#   write.xlsx(x = out[[i]], file = "data/v3/within.cluster.DEGs.Migr1.vs.Mpl.LARGE.GENERIC.CLUSTERS.xlsx",
#              sheetName=temp.name, append = T)
# }
# write.xlsx(x = de.genes, file = "data/v3/within.cluster.DEGs.Migr1.vs.Mpl.LARGE.GENERIC.CLUSTERS.xlsx",
#            sheetName='All-Clusters', append = T)
```


## DE Expression within Group: Enriched vs Non.Enriched fine labels
...
## DE Expression within Group: Enriched vs Non.Enriched generic labels
...
#   print(i)
# 
#   temp.subset <- subset(wbm, v7.labels == i & Experiment == condition)
# 
#   x <- FindMarkers(temp.subset,
#                    ident.1 = 'Enriched',
#                    ident.2 = 'Not Enriched',
...
#            file = "data/v3/within.cluster.and.Experiment.DEGs.Enriched.vs.NotEnriched.GENERIC.LABEL.xlsx",
#            sheetName='All-Clusters', append = T)
...
```{r}
DimPlot
table(wbm$v7.labels, wbm$Condition2)
```

# Splitting up the MKs

```{r}
wbm$umap1 <- wbm@reductions$umap@cell.embeddings[,1]
wbm$umap2 <- wbm@reductions$umap@cell.embeddings[,2]

wbm$v8.labels <- ifelse(wbm$v7.labels != 'MK', wbm$v7.labels,
                        ifelse(wbm$umap1 < 0, 'MK-1','MK-2'))

temp.subset <- subset(wbm, v7.labels == 'MK')


DimPlot(temp.subset)

DimPlot(temp.subset, group.by = 'v8.labels')

table(temp.subset$Condition2, temp.subset$v8.labels)

DimPlot(wbm, group.by = 'v8.labels')
```
...
wbm$figure.labels <- ifelse(wbm$v7.labels %in% c('MEP/MCP','MEP/ERP'),
                            ifelse(wbm$v7.labels == 'MEP/MCP', 'MEP-1','MEP-2'),
...
wbm$figure.labels <- factor(wbm$figure.labels,
                            levels = levels(as.factor(wbm$figure.labels))[c(5,3,1,2,10,9,4,8,6,7)])
```
...
```{r}
DimPlot(wbm, label = T, repel = T, cols = color_pal, group.by = 'figure.labels',
        pt.size = .001) +
  theme_bw() +
  theme(text = element_text(size = 6, family = 'sans'))

ggsave('./figures/version7.for.v3/1a.umap.pdf', device = 'pdf',
...
wbm$Condition3 <- ifelse(wbm$Condition2 == 'Migr1', 'MigR1',
                         ifelse(wbm$Condition2 == 'enrMigr1', 'enrMigR1',
                                ifelse(wbm$Condition2 == 'Mpl', 'MPLW515L', 'enrMPLW515L')))

wbm$Condition3 <- factor(wbm$Condition3,
                         levels = levels(as.factor(wbm$Condition3))[c(3,4,1,2)])

DimPlot(wbm, cols = color_pal, group.by = 'figure.labels', 
        split.by = 'Condition3', ncol =2, pt.size =  0.05) +
  theme_bw() + NoLegend() +
  theme(text = element_text(size = 6, family = 'sans'))

ggsave('./figures/version7.for.v3/1b.split.umap.pdf', device = 'pdf',
...
```{r}
markers <- c('Ngp','Kit','Ighd','Flt3','Ccl5','Cd68','Alas2','Pf4','Itga2b','Gata2')
...
ggsave('./figures/version7.for.v3/1c.dot.plot.markers.pdf', device = 'pdf',
       height = 3, width = 4, units = 'in', dpi = 600)
```

```{r}
# temp.markers <- FindMarkers(wbm,
#                             ident.1 = 'MEP/ERP',
#                             group.by = 'v7.labels',
#                             logfc.threshold = log(2),
#                             test.use = 'MAST')
# 
# temp.markers <- temp.markers[order(temp.markers$avg_logFC, decreasing = T),]
# head(temp.markers)
```
...
```{r}
table(wbm$Condition2, wbm$figure.labels)
...
ggsave('./figures/version7.for.v3/1d.non.enriched.quant.pdf', device = 'pdf',
       height = 3, width = 4, units = 'in', dpi = 600)
...
```{r}
temp.table <- as.data.frame(table(wbm$Condition, wbm$figure.labels))
...
temp.table$State2 <- factor(temp.table$State2,
                           levels = c('MigR1','MPLW515L','enrMigR1','enrMPLW515L'))
...
ggsave('./figures/version7.for.v3/supp.all.quant.table.pdf', device = 'pdf',
       height = 3, width = 5, units = 'in', dpi = 600)
```


## e) From normal to enriched (maybe supplement)

?? Percentage vs Total Count!!
...
temp.table$State <- ifelse(temp.table$State == 'Migr1', 'MigR1','enrMigR1')

temp.table$State <- factor(temp.table$State,
                           levels = c('MigR1','enrMigR1'))
...
ggsave('./figures/version7.for.v3/1sup.migr1.enriched.quant.pdf', device = 'pdf',
...
temp.table$State <- ifelse(temp.table$State == 'Mpl', 'MPLW515L','enrMPLW515L')

temp.table$State <- factor(temp.table$State,
                           levels = c('MPLW515L','enrMPLW515L'))
...
ggsave('./figures/version7.for.v3/1sup.mpl.enriched.quant.pdf', device = 'pdf',
       height = 3, width = 6, units = 'in', dpi = 600)
...
# ?DoHeatmap
# 
de.genes <- read.xlsx('./data/v3/within.cluster.DEGs.Migr1.vs.Mpl.LARGE.GENERIC.CLUSTERS.xlsx', sheetName = 'All-Clusters')
# 
# wbm$heatmap.group <- paste0(wbm$v7.labels,'_',wbm$Experiment)
# 
# unique(wbm$heatmap.group)
# 
# clusters.to.use <- unique(de.genes$cluster)
# 
# temp.subset <- subset(wbm, v7.labels %in% clusters.to.use)
# 
# summary(as.factor(temp.subset$heatmap.group))
# 
# tmp.md <- temp.subset@meta.data
# 
# row.subset <- NA
# 
# for (i in unique(tmp.md$heatmap.group)){
#   
#   temp.df <- tmp.md[tmp.md$heatmap.group == i,]
#   
#   # print(paste0(i,' with dim ', dim(temp.df)[1]))
#   
#   if (dim(temp.df)[1] < 50){
#     # print('Using All')
#     rows <- rownames(temp.df)
#   }
#   else{
#     # print('Subsetting')
#     rows <- sample(rownames(temp.df),50)
#   }
#   row.subset <- c(row.subset,rows)
#   # print(length(row.subset))
# }
# 
# length(row.subset)
# 
# wbm$cell.id <- rownames(wbm@meta.data)
# 
# temp.subset <- subset(wbm, cell.id %in% row.subset)
# 
summary(as.factor(de.genes$cluster))
# 
# DoHeatmap(temp.subset, features = de.genes$gene, group.by = 'heatmap.group')
```

Above was code to do the generic label DE, but there are not a ton of genes showing up. Instead maybe just show a Heatmap for MEP/MCP
...
ggsave('./figures/version7.for.v3/1e.mep.1.heatmap.pdf', device = 'pdf',
       height = 3, width = 4, units = 'in', dpi = 600)
```
...
cnts <- temp.subset@assays$RNA

cnts <- cnts[rownames(cnts) %in% de.genes$gene,]
...
# 
# cnts.test <- cnts[1:5, c(1:5,579)]
# gather(cnts.test, key = 'cell',value = 'Expression', - gene)

cnts <- gather(cnts, key = cell, value = 'Expression', - gene)

ggplot(cnts, aes(x = cell, y = gene, fill = Expression)) + geom_tile()
```


```{r}
# write.table(de.genes$gene,'./data/v3/go_term/large.clustering.mep.deg.all.txt', row.names = F, quote = F)
# 
# write.table(de.genes[de.genes$avg_logFC > 0,]$gene, './data/v3/go_term/large.clustering.mep.deg.up.in.Migr1.txt',
#             row.names = F, quote = F)
# 
# write.table(de.genes[de.genes$avg_logFC < 0,]$gene, './data/v3/go_term/large.clustering.mep.deg.up.in.Mpl.txt',
#             row.names = F, quote = F)
# 
# write.table(rownames(wbm), './data/v3/go_term/all.genes.expressed.txt',
#             row.names = F, quote = F)
```
...
ggsave('./figures/version7.for.v3/supp.umap.with.all.clusters.pdf', device = 'pdf',
       height = 4, width = 5, units = 'in', dpi = 600)
```


### Comparison to Human wbm

Doesn't show us anything of interest

```{r}
# converting from mouse to human gene names
...
ortho.list <- read.table('../mouse.human.orthologs.unique.tsv', sep = '\t')
head(ortho.list)
colnames(ortho.list) <- ortho.list[1,]

ortho.list <- ortho.list[-1,]

print('The number of genes in our dataset with unique human orthologs')
summary(rownames(wbm) %in% ortho.list$`Gene name`)

temp.subset <- wbm[rownames(wbm) %in% ortho.list$`Gene name`,]
dim(temp.subset)

Idents(temp.subset) <- temp.subset$figure.labels

for (i in head(rownames(wbm.human.centroids),10)){
  print(i)
  print(ortho.list[ortho.list$`Gene name` ==i, ]$`Human gene name`)
}

wbm.human.centroids <- AverageExpression(temp.subset)$RNA

rownames(wbm.human.centroids) <- toupper(rownames(wbm.human.centroids))

# loading hcl data

hcl.centroids <- read.table('/Users/dfhannum/Desktop/CZI_Ovary/data/HCL_data/HCL_centroids.txt', sep = '\t')

# checking the overlap in genes

dim(wbm.human.centroids)
dim(hcl.centroids)



summary(toupper(VariableFeatures(temp.subset)) %in% rownames(hcl.centroids))

summary(rownames(wbm.human.centroids) %in% rownames(hcl.centroids))

all.genes <- rownames(wbm.human.centroids)[rownames(wbm.human.centroids) %in% rownames(hcl.centroids)]

hv.genes <- toupper(VariableFeatures(temp.subset))[toupper(VariableFeatures(temp.subset)) %in%
                                                     rownames(hcl.centroids)]
length(all.genes)
length(hv.genes)
```

```{r}
wv <- wbm.human.centroids[rownames(wbm.human.centroids) %in% all.genes,]
hv <- hcl.centroids[rownames(hcl.centroids) %in% all.genes,]

wv$gene <- rownames(wv)
hv$gene <- rownames(hv)
mg <- merge(wv, hv, by = 'gene')
rownames(mg) <- mg$gene
mg <- mg[,-1]

hcl.cor <- cor(mg)
# hcl.cor[1:6,1:6]
key.columns <- colnames(hcl.cor)[1:10]
hcl.cor <- hcl.cor[rownames(hcl.cor) %in% key.columns, !(colnames(hcl.cor) %in% key.columns)]
# hcl.cor[,1:10]
hcl.cor <- as.data.frame(hcl.cor)
correlation <- gather(hcl.cor, Cluster1, Correlation, factor_key = T)

correlation$Cluster2 <- rep(key.columns, dim(correlation)[1]/length(key.columns))

corr.out <- split(correlation, f = correlation$Cluster2)
...
top3.hits <- top3.hits[-1,c(3,4,1,2)]

colnames(top3.hits) <- c('Our Clusters','GeneSet','HCL Cluster','Correlation')

rownames(top3.hits) <- 1:length(rownames(top3.hits))

# top3.hits <- top3.hits[c(13:24,10:12,4:6,1:3,7:9),]

correlation <- correlation[correlation$Cluster1 %in% top3.hits$`HCL Cluster`,]

correlation$OurClusters <- factor(correlation$Cluster2,
                                     levels = levels(as.factor(wbm$figure.labels))[c(5,3,1,2,10,9,4,8,6,7)])

# cluster1.levels <- rev(unique(with(correlation, correlation[order(OurClusters, Correlation),])$Cluster1))

correlation$`HCL Clusters` <- factor(correlation$Cluster1, levels = unique(top3.hits$`HCL Cluster`))

ggplot(correlation, aes(x = OurClusters, y = `HCL Clusters`, fill = Correlation)) +
  geom_tile() + 
  scale_fill_gradient2(low = 'blue', mid = 'white', high = 'red', midpoint = 0.5) +
  theme(axis.text.x = element_text(angle = 60, hjust = 1))

# ggsave('figures/sample1.compared.to.all.HCL.centroids.top3.hits.using.all.genes.png', device = 'png',
#        units = 'in', width = 8, height = 5, dpi = 400)

kable(top3.hits)
```


### Saving an object to use with the Azimuth tool from the Satijia lab

(link)[http://azimuth.satijalab.org/app/azimuth]

Said they were to divergent to map to their clusters

```{r}
# temp.counts <- temp.subset@assays$RNA@counts
# rownames(temp.counts) <- toupper(rownames(temp.counts))
# 
# saveRDS(temp.counts, './data/v3/count.matrix.with.human.gene.orthologs.rds')
```
...
ggsave('figures/version7.for.v3/1sup.immgen.cluster.centroid.top.hit.pdf', device = 'pdf',
       units = 'in', width = 6, height = 4, dpi = 600)
```
...
ggsave('figures/version7.for.v3/1sup.mouseRNA.cluster.centroid.top.hit.pdf', device = 'pdf',
       units = 'in', width = 5, height = 4, dpi = 600)
```
